...
function ClientOrderForm(props) {
    ...
    useEffect(() => {
        api_getProducts()
            ...
            .catch((e) => addMessage({message: e, type: 'danger'}));
    }, [onHide]);
    ...
    const addOrder = () => {
        if (productsClient.length === 0) {
            addMessage("", 'Complete add at least one product', 'danger');
        } else if (insertProduct) {
            addMessage("", 'Complete the addition of the last product', 'danger');
        } else {
            //send the request
            ...
            api_addOrder(orderClient)
                .then((id) =>
                    addMessage("", 'Order ' + id + ' emitted with success ',
                        'success')
                )
                .catch((err) => {
