  ...
  async (req, res) => {
    ...
    try {
      ...
      basket.forEach(async (p) => {
        ...
        await orderDAO.insertProductRequest(requestId, p.productId, p.quantity);
        // clear basket
        await basketDAO.removeProductFromBasket(userId, p.productId);
      });

      await orderDAO.checkBalanceAndSetStatus(requestId);
